...
- **Discord slash commands work but the bot is not visible in the member list / sends fail with `403 Missing Access`**: the app was likely installed with `applications.commands` only, so slash commands exist but the bot user was not added to the guild. Re-invite with `scope=bot+applications.commands` and permissions `274878286912`, then make sure the bot role can View Channel, Send Messages, and Read Message History in the target channel.
