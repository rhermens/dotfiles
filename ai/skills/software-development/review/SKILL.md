...
- For NestJS/CQRS command/query handlers, flag single-use top-level helper functions that are only implementation details of one handler, especially when they build Mongo aggregation fragments, query predicates, or pipeline stages. Prefer inline pipelines when they are easier to read end-to-end; if extraction is justified, handler-local helpers should usually be private methods rather than module-scope functions. Treat vague types such as `Record<string, unknown>` around Mongo expressions as a sign the abstraction may be fighting TypeScript and harming readability.
- For domain/entity predicates that are duplicated or moved into a query/command handler, check whether the closest entity already owns a sibling getter/method (for example manage-facing vs customer-facing variants). Flag handler-local copies of domain timing/status semantics when they would be more consistent and reusable as entity getters next to the existing domain predicate.
